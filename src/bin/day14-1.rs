...
// roughly in the shape of a parabolic reflector dish, each individual mirror seems to
// be pointing in slightly the wrong direction. If the dish is meant to focus light,
...
// reflector dish, maybe you can go where it's pointing and use the light to fix the
// lava production.
//
...
// platform is covered in large rocks of various shapes. Depending on their position,
...
// rounded rocks (O) will roll when the platform is tilted, while the cube-shaped rocks
...
// rows from the rock to the south edge of the platform, including the row the rock is
// on. (Cube-shaped rocks (#) don't contribute to load.) So, the amount of load caused
// by each rock in each row is as follows:
//
// OOOO.#.O.. 10
// OO..#....#  9
// OO..O##..O  8
