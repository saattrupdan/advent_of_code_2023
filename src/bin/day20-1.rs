...
// machines. Now, they just need to send the command to boot up the machines and get
// the sand flowing again.
//
// The machines are far apart and wired together with long cables. The cables don't
// connect to the machines directly, but rather to communication modules attached to
// the machines that perform various initialization tasks and also act as communication
// relays.
//
...
// b, and c. Each of these modules is a flip-flop module (as indicated by the %
// prefix). a outputs to b which outputs to c which outputs to another module named
// inv. inv is a conjunction module (as indicated by the & prefix) which, because it
// has only one input, acts like an inverter (it sends the opposite of the pulse type
...
// flip-flops are on and con remembers a high pulse from each of its two inputs,
