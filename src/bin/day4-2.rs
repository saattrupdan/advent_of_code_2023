...
fn main() {
    ...
    let mut line_content: Vec<&str>;
    let mut number_sequences: Vec<&str>;
    let mut winning_numbers: Vec<u32>;
    let mut my_numbers: Vec<u32>;
    let mut num_winners: usize;
    let mut card_factor: u32;

    // Initialise a hashmap to keep track of the number of scratchcards for each card.
    // We start with one scratchcard for each card.
    let mut num_scratchcards: HashMap<usize, u32> = HashMap::new();
    for card_idx in 0..data.lines().count() {
        num_scratchcards.insert(card_idx, 1);
    }

    for (card_idx, line) in data.lines().enumerate() {
        // Extract the winning numbers and my numbers from the line.
        line_content = line.split(":").collect();
        number_sequences = line_content[1].split("|").collect();
        winning_numbers = number_sequences[0]
            .trim()
            .split(" ")
            .filter(|x| x != &"")
            .map(|x| x.parse::<u32>().unwrap())
            .collect();
        my_numbers = number_sequences[1]
            .trim()
            .split(" ")
            .filter(|x| x != &"")
            .map(|x| x.parse::<u32>().unwrap())
        ...
        // For each winner, add a scratchcard to following cards.
        num_winners = 0;
        card_factor = num_scratchcards.get_mut(&card_idx).unwrap().clone();
        for my_number in my_numbers {
            if winning_numbers.contains(&my_number) {
                num_winners += 1;
                num_scratchcards
                    .entry(card_idx + num_winners)
                    .and_modify(|x| *x += card_factor);
            }
        }
    }

    // The answer is the sum of the number of scratchcards for each card.
    let answer: u32 = num_scratchcards.values().sum();
