...
// 0,1,6~2,1,6
// 1,1,8~1,1,9
// Each line of text in the snapshot represents the position of a single brick at the
// time the snapshot was taken. The position is given as two x,y,z coordinates - one
// for each end of the brick - separated by a tilde (~). Each brick is made up of a
// single straight line of cubes, and the Elves were even careful to choose a time for
// the snapshot that had all of the free-falling bricks at integer positions above the
// ground, so the whole snapshot is aligned to a three-dimensional cube grid.
//
...
//
// Lines like 0,0,10~1,0,10 or 0,0,10~0,1,10 both represent bricks that are two cubes
// in volume, both oriented horizontally. The first brick extends in the x direction,
...
// the same position, so a falling brick will come to rest upon the first other brick
// it encounters.
//
// Here is the same example again, this time with each brick given a letter so it can
// be marked in diagrams:
//
// 1,0,1~1,2,1   <- A
// 0,0,2~2,0,2   <- B
// 0,2,3~2,2,3   <- C
...
// Your first task is to figure out which bricks are safe to disintegrate. A brick can
// be safely disintegrated if, after removing it, no other bricks would fall further
// directly downward. Don't actually disintegrate any bricks - just determine what
// would happen if, for each brick, only that brick were disintegrated. Bricks can be
// disintegrated even if they're completely surrounded by other bricks; you can squeeze
...
// would both fall.
// Brick B can be disintegrated; the bricks above it (D and E) would still be supported
// by brick C.
// Brick C can be disintegrated; the bricks above it (D and E) would still be supported
// by brick B.
// Brick D can be disintegrated; the brick above it (F) would still be supported by
// brick E.
// Brick E can be disintegrated; the brick above it (F) would still be supported by
// brick D.
...
// consider disintegrating a single brick; how many bricks could be safely chosen as
// the one to get disintegrated?
