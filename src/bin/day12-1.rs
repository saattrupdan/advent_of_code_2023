...
// for the hot springs, weren't you?" You indicate that this definitely looks like hot
// springs to you.
//
...
// they're not actually sure which springs would even be safe to use! Worse yet, their
// condition records of which springs are damaged (your puzzle input) are also damaged!
...
// (#). This is the part of the condition records that is itself damaged; for some
// springs, it is simply unknown (?) whether the spring is operational or damaged.
//
// However, the engineer that produced the condition records also duplicated some of
// this information in a different format! After the list of springs for a given row,
// the size of each contiguous group of damaged springs is listed in the order those
// groups appear in the row. This list always accounts for every damaged spring, and
// each number is the entire size of its contiguous group (that is, groups are always
// separated by at least one operational spring: #### would always be 4, never 2,2).
//
...
// three unknown springs must be broken, then operational, then broken (#.#), making
// the whole row #.#.###.
//
...
// first number would have to be 4 or higher). However, the remaining run of unknown
