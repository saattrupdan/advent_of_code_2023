...
// that there is indeed normally a large pile of sand somewhere near here, but you
// don't see anything besides lots of water and the small island where the ferry has
// docked.
//
...
// the race. The boat won't move; it will have traveled 0 millimeters by the end of the
// race.
...
fn main() {
    ...
    let race_durations: Vec<u32> = lines
        .next()
        .unwrap()
        .split(":")
        .last()
        ...
        .collect();
    let record_distances: Vec<u32> = lines
        .next()
        .unwrap()
        .split(":")
        .last()
        ...
        .filter_map(|x| x.trim().parse::<u32>().ok())
        .collect();
    let races: Vec<(u32, u32)> = race_durations
        .iter()
