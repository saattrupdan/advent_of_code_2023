...
// and running since the lavafall came back. However, to catch up with the large
// backlog of parts requests, the factory will also need a large supply of lava for a
// while; the Elves have already started creating a large lagoon nearby for this
// purpose.
//
...
// cubes as they go. The directions are given as seen from above, so if "up" were
// north, then "right" would be east, and so on. Each trench is also listed with the
// color that the edge of the trench should be painted as an RGB hexadecimal color
// code.
//
...
// .#....#
// .######
// At this point, the trench could contain 38 cubic meters of lava. However, this is
// just the edge of the lagoon; the next step is to dig out the interior so that it is
// one meter deep as well:
//
// #######
// #######
// #######
...
// the interior is dug out, the edges are also painted according to the color codes in
// the dig plan.
//
...
fn main() {
    // let data = include_str!("../../data/day18.txt");
