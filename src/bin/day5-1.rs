...
// 60 56 37
// 56 93 4
// The almanac starts by listing which seeds need to be planted: seeds 79, 14, 55, and
// 13.
//
// The rest of the almanac contains a list of maps which describe how to convert
// numbers from a source category into numbers in a destination category. That is, the
// section that starts with seed-to-soil map: describes how to convert a seed number
// (the source) to a soil number (the destination). This lets the gardener and his team
// know which soil to use with which seeds, which water to use with which fertilizer,
// and so on.
//
...
// 50, so its two values are 50 and 51. With this information, you know that seed
// number 98 corresponds to soil number 50 and that seed number 99 corresponds to soil
// number 51.
//
...
fn main() {
    ...
    let mut all_maps: HashMap<&str, Vec<Vec<u32>>> = HashMap::new();
    let mut active_map_name: Option<&str> = None;
    let mut active_map: Vec<u32>;
    let mut id_offset: u32;
    ...
    for type_id in 0..types.len() - 1 {
        ...
        for (id_idx, id) in old_ids.iter().enumerate() {
            let map_name = format!("{}-to-{}", types[type_id], types[type_id + 1]);
            for map_line in all_maps.get(map_name.as_str()).unwrap() {
                if id >= &map_line[1] && id - map_line[1] < map_line[2] {
                    id_offset = id - map_line[1];
                    ids[id_idx] = map_line[0] + id_offset;
                }
            }
        }
    }

    let answer = ids.iter().min().unwrap();
